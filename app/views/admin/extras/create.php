<?php
/**
 * Admin Extras Create View
 */
?>

<div class="page-header">
    <h1 class="page-title"><?php _e('add_extra'); ?></h1>
...
<form action="<?php echo $adminUrl; ?>/extras/create" method="post" enctype="multipart/form-data">
    <div class="row">
        <div class="col-md-8">
            <div class="card">
                            ...
                            <div class="language-tab-content <?php echo $index === 0 ? 'active' : ''; ?>" data-lang="<?php echo $lang['code']; ?>">
                                <div class="form-group">
                                    ...
                                    <input type="text" id="name_<?php echo $lang['id']; ?>" name="details[<?php echo $lang['id']; ?>][name]" 
                                           ...
                                           value="<?php echo isset($details[$lang['id']]['name']) ? htmlspecialchars($details[$lang['id']]['name']) : ''; ?>">
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea id="description_<?php echo $lang['id']; ?>" name="details[<?php echo $lang['id']; ?>][description]" 
                                              class="form-control" rows="3"><?php echo isset($details[$lang['id']]['description']) ? htmlspecialchars($details[$lang['id']]['description']) : ''; ?></textarea>
                                </div>
                            </div>
                        <?php endforeach; ?>
                    </div>
                ...
                <div class="card-body">
                    <div class="form-group">
                        ...
                        <select id="pricing_type" name="pricing_type" class="form-control" onchange="togglePricingFields()">
                            <option value="fixed_group" <?php echo ($pricingType ?? 'fixed_group') === 'fixed_group' ? 'selected' : ''; ?>><?php _e('fixed_group_price'); ?></option>
                            <option value="per_person" <?php echo ($pricingType ?? '') === 'per_person' ? 'selected' : ''; ?>><?php _e('per_person_price'); ?></option>
                            <option value="tiered" <?php echo ($pricingType ?? '') === 'tiered' ? 'selected' : ''; ?>><?php _e('tiered_pricing'); ?></option>
                        </select>
                ...
                <div class="card-body">
                    <div class="form-group">
                        ...
                        <select id="category" name="category" class="form-control">
                            ...
                            <option value="transportation" <?php echo ($category ?? '') === 'transportation' ? 'selected' : ''; ?>><?php _e('transportation'); ?></option>
                            <option value="meals" <?php echo ($category ?? '') === 'meals' ? 'selected' : ''; ?>><?php _e('meals'); ?></option>
                            <option value="activities" <?php echo ($category ?? '') === 'activities' ? 'selected' : ''; ?>><?php _e('activities'); ?></option>
                            <option value="accommodation" <?php echo ($category ?? '') === 'accommodation' ? 'selected' : ''; ?>><?php _e('accommodation'); ?></option>
                            <option value="other" <?php echo ($category ?? '') === 'other' ? 'selected' : ''; ?>><?php _e('other'); ?></option>
                        </select>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" id="order_number" name="order_number" class="form-control" 
                               min="0" value="<?php echo isset($orderNumber) ? $orderNumber : '0'; ?>">
                    ...
                    <div class="form-group">
                        <div class="form-check">
                            <input type="checkbox" id="is_active" name="is_active" value="1" class="form-check-input" 
                                   <?php echo ($isActive ?? true) ? 'checked' : ''; ?>>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (addTierBtn && tiersContainer) {
        addTierBtn.addEventListener('click', function() {
            tierCount++;
            const tierHtml = `
                <div class="pricing-tier" data-tier="${tierCount}">
                    <div class="row">
                        <div class="col-4">
                            <input type="number" name="pricing_tiers[${tierCount}][min_persons]" 
                                   class="form-control" placeholder="Min Persons" min="1" required>
                        </div>
                        <div class="col-4">
                            <input type="number" name="pricing_tiers[${tierCount}][max_persons]" 
                                   class="form-control" placeholder="Max Persons" min="1">
                        </div>
                        <div class="col-3">
                            <input type="number" name="pricing_tiers[${tierCount}][price_per_person]" 
                                   class="form-control" placeholder="Price" step="0.01" min="0" required>
                        </div>
                        <div class="col-1">
                            <button type="button" class="btn btn-danger btn-sm remove-tier-btn">
        ...
        // Remove tier functionality (event delegation)
